...
class BrewParser:
    ...
    def __init__(self) -> None:
        ...
        self.headers = {
            "User-Agent": "brew-parser/1.0 (https://github.com/yourusername/brew-parser)"
        }
    ...
    def fetch_all_formulas(self) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(formulas)} formulas")
            return formulas
    ...
    def filter_new_formulas(self, formulas: List[Dict], days: int = 7) -> List[Dict]:
        """
    ...
    def format_as_markdown(self, formulas: List[Dict]) -> str:
        """
    ...
    def update_stored_formulas(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if old_hash and old_hash == new_hash:
                ...
                return True, f"Successfully updated formula data. Total formulas: {len(formulas)}"
                
        except Exception as e:
            logger.error(f"Failed to update formula data: {e}")
            return False, f"Failed to update: {str(e)}"
    
    def load_stored_formulas(self) -> Optional[List[Dict]]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to load stored formulas: {e}")
            return None
    
    def compare_formulas(self, old_formulas: List[Dict], new_formulas: List[Dict]) -> Dict[str, List[Dict]]:
        """
        ...
        Example Return:
            {
                ...
                'updated': [{'name': 'tool', 'old_version': '1.0', 'new_version': '2.0', ...}]
            }
        """
    ...
    def format_diff_as_table(self, diff: Dict[str, List[Dict]]) -> None:
        """
...
def handle_diff_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        console.print(f"[bold red]Error:[/bold red] Failed to fetch data from Homebrew: {e}")
        sys.exit(1)
    except KeyboardInterrupt:
...
Examples:
  ...
  %(prog)s new --limit 5      # Show only 5 newest formulas
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Limit number of formulas to display"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    update_parser = subparsers.add_parser(
        'update', 
        help='Update local formula database with current data'
    )
    ...
    diff_parser = subparsers.add_parser(
        'diff', 
        help='Show all changes (added/removed/updated) since last update'
    )
    ...
    new_parser = subparsers.add_parser(
        'new', 
        help='Show only newly added formulas since last update'
    )
    new_parser.add_argument(
        "--limit",
        type=int,
        help="Limit number of new formulas to display"
    )
    new_parser.add_argument(
        "--debug",
        action="store_true",
