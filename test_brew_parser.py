...
from brew_parser import BrewParser, handle_update_command, handle_diff_command, handle_new_command
...
class TestBrewParser:
    ...
    def test_load_stored_formulas_success(self, parser, tmp_path):
        ...
        # Create test data
        test_data = [{"name": "tool1"}, {"name": "tool2"}]
        with open(parser.stored_formulas_path, 'w') as f:
    ...
    def test_format_diff_as_table(self, mock_print, parser):
        ...
        # Create sample diff
        diff = {
            'added': [{"name": "new-tool", "versions": {"stable": "1.0"}, "desc": "New tool"}],
            'removed': [{"name": "old-tool", "versions": {"stable": "0.9"}, "desc": "Old tool"}],
            'updated': [{
                "name": "updated-tool",
                "old_version": "1.0",
                "new_version": "2.0",
...
class TestSubcommands:
    ...
    @patch('brew_parser.BrewParser')
    @patch('brew_parser.Console')
    def test_handle_diff_command_no_baseline(self, mock_console_class, mock_parser_class):
    ...
    @patch('brew_parser.BrewParser')
    @patch('brew_parser.Console')
    def test_handle_new_command_with_results(self, mock_console_class, mock_parser_class):
    ...
    @patch('brew_parser.BrewParser')
    @patch('brew_parser.Console')
    def test_handle_new_command_no_new_formulas(self, mock_console_class, mock_parser_class):
...
class TestMainFunction:
    ...
    def test_main_with_api_error(self, mock_parser_class):
        ...
        mock_parser = Mock()
        mock_parser_class.return_value = mock_parser
        mock_parser.fetch_all_formulas.side_effect = requests.RequestException("Network error")
